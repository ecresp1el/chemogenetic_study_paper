...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create a single 1x4 figure with all three subgroups per technology "
            "using mean (dots) and SEM (error bars)."
        )
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        "--output",
        default=str(REPO_ROOT / "output" / "plots" / "three_subgroups_1x4.png"),
        help="Path for output figure.",
    )
    parser.add_argument(
        "--summary-output",
        default=str(REPO_ROOT / "output" / "plots" / "three_subgroups_mean_sem.csv"),
        help="Path for summary CSV used in the plot.",
    )
    parser.add_argument(
        "--dpi",
        type=int,
...
def main() -> None:
    ...
    for idx, technology in enumerate(TECH_ORDER):
        ...
        ann_lines = []
        for g in GROUPS:
            cond = tech_map.get(g, "NA")
            ann_lines.append(f"{GROUP_SHORT[g]}: {cond}")
        ax.text(
            0.02,
            0.98,
            "\n".join(ann_lines),
    ...
    fig.suptitle("Three Subgroups per Technology: Mean (dots) +/- SEM (bars)", y=1.08)
    fig.tight_layout()
